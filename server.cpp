...
static bool try_one_request(Conn *conn) {
  if (conn->incoming.size() < 4) {
    return false; // need to read more bytes
  }
  uint32_t len = 0;
  ...
  if (4 + len > conn->incoming.size()) {
    return false; // need to finish reading message body
  }
  ...
  // application logic done -- remove request from incoming buffer
...
static void handle_read(Conn *conn) {
  ...
  while (try_one_request(conn)) { } // loop for request pipelining

  if (conn->outgoing.size() > 0) {  // response ready to be written to client
